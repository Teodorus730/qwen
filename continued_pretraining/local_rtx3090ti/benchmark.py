...
def worker(args) -> int:
    ...
    try:
        ...
        batch_iter = cyclic_batches(
            blocks, args.worker_batch_size, seed=cfg.training.seed
        )
        ...
        result = {
            **common,
            ...
            "packed_blocks_available": blocks_report["blocks"],
...
def save_plot(path: Path, rows: list[dict], fit: dict | None) -> None:
    ...
    axes[0].set(
        ...
        title="Training memory above desktop baseline",
    )
    ...
    figure.suptitle(
        "StellaAthena Qwen3-0.6B — RTX 3090 Ti, "
...
def orchestrator(args) -> int:
    ...
    # Prepare once so workers measure training rather than network/tokenisation.
    slice_path, blocks_path = project_data_paths(cfg)
    slice_report = materialize_slice(cfg.data, slice_path)
    tokenizer = load_tokenizer(cfg)
    _, blocks_report = load_or_build_blocks(
        cfg.data, slice_path, tokenizer, blocks_path
    )
    print(f"[prepared] {slice_report}", flush=True)
    print(f"[prepared] {blocks_report}", flush=True)
    ...
    summary = {
        ...
        "memory_definition": {
            "primary": (
                "incremental_device_peak_mib = total CUDA memory minus the "
                "minimum cudaMemGetInfo free memory sampled at forward, "
                "backward, clipping and optimizer phases, minus the device "
                "usage before the worker loaded the model"
            ),
            "torch_secondary": (
                "peak_allocated_mib = torch.cuda.max_memory_allocated; it "
                "excludes bitsandbytes and other non-PyTorch allocations"
            ),
        },
        "linear_fit": fit,
        "rows": rows,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Subprocess-isolated micro-batch VRAM/throughput benchmark."
    )
