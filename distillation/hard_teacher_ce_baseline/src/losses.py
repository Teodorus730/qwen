...
def hard_teacher_ce_loss(student_logits, teacher_logits,
                         ignore_mask: torch.Tensor | None = None):
    """Temperature->0 teacher baseline.

    Instead of matching the full soft teacher distribution, collapse the teacher
    to its argmax token at every position and train the student with ordinary CE
    against that hard target.
    """
    B, S, V = student_logits.shape
    teacher_targets = teacher_logits.detach().argmax(dim=-1)  # (B, S)

    if ignore_mask is not None:
        teacher_targets = teacher_targets.masked_fill(ignore_mask, -100)

    # Keep student logits in their incoming dtype (bf16 on CUDA). Unlike soft KD
    # we do not need fp32 probabilities here; casting the full B*S*V tensor to
    # fp32 roughly doubles peak memory and can trigger Windows sysmem fallback.
    ce = F.cross_entropy(student_logits.reshape(-1, V),
                         teacher_targets.reshape(-1), ignore_index=-100)

    with torch.no_grad():
        valid = teacher_targets != -100
        pred = student_logits.argmax(dim=-1)
        denom = valid.sum().clamp_min(1)
        acc = ((pred == teacher_targets) & valid).sum().float() / denom

    return ce, {
        "loss": ce.detach(),
        "kd": torch.zeros((), device=student_logits.device),
        "ce": ce.detach(),
        "teacher_argmax_acc": acc.detach(),
        "teacher_argmax_nll": ce.detach(),
    }
