...
Usage:
    ...
    python -m src.distill --config configs/main_synthetic_ce_epoch_xpu_qwen3.5_0.8b.yaml --alpha 0.1
"""
...
def save_training_artifact(cfg: RunConfig, student, out: Path, device,
                           optimizer_steps: int, processed_examples: int | None,
                           final_metrics: dict | None) -> Path | None:
    """Save a PEFT adapter, or preserve the existing full-FT save behaviour."""
    if cfg.lora_r <= 0:
        if cfg.save_adapter:
            raise ValueError("save_adapter requires lora_r > 0")
        if cfg.save_student:
    ...
    if not (cfg.save_student or cfg.save_adapter):
        return None

    from peft import PeftModel
    if not isinstance(student, PeftModel):
        raise TypeError("LoRA run did not produce a PeftModel")
    adapter_dir = (Path(cfg.adapter_output_dir)
                   if cfg.adapter_output_dir else out / "adapter")
    student.save_pretrained(adapter_dir, safe_serialization=True)
    metadata = {
        "base_model": cfg.model_name,
        "base_model_revision": cfg.base_model_revision,
        "source_run": str(out),
        "noise_alpha": cfg.alpha,
        "noise_seed": cfg.noise_seed,
        "training_seed": (cfg.training_seed if cfg.training_seed is not None
                          else cfg.noise_seed),
        "objective": cfg.objective,
        "dataset_path": cfg.local_jsonl,
        "dataset_sha256": sha256_file(cfg.local_jsonl),
        "lora_rank": cfg.lora_r,
        "lora_alpha": cfg.lora_alpha,
        "lora_dropout": cfg.lora_dropout,
        "target_modules": cfg.lora_target_modules,
        "dtype": str(model_dtype(device)).removeprefix("torch."),
        "device": str(device),
        "versions": {
...
def load_models(cfg: RunConfig, device):
    ...
    revision_kwargs = ({"revision": cfg.base_model_revision}
                       if cfg.base_model_revision else {})
    tok = AutoTokenizer.from_pretrained(cfg.model_name, **revision_kwargs)
    if tok.pad_token_id is None:
        tok.pad_token = tok.eos_token
    dtype = model_dtype(device)
    teacher = AutoModelForCausalLM.from_pretrained(
        cfg.model_name, dtype=dtype, **revision_kwargs)
...
def main():
    ...
    ap.add_argument("--run_name", "--run-name", dest="run_name", type=str,
                    default=None)
    ap.add_argument("--max_seconds", "--max-seconds", dest="max_seconds",
                    type=float, default=None)
    ap.add_argument("--device", type=str, default=None)
    ap.add_argument("--save-adapter", dest="save_adapter", action="store_true",
                    default=None)
    ap.add_argument("--adapter-output-dir", dest="adapter_output_dir",
    ...
    cfg = load_config(args.config, {
        ...
        "device": args.device, "save_adapter": args.save_adapter,
    ...
    torch.manual_seed(
        cfg.training_seed if cfg.training_seed is not None else cfg.noise_seed)
    ...
    artifact = save_training_artifact(
        cfg, student, out, device, optimizer_steps,
        data_audit["source_rows"] if data_audit else None, final_metrics)
    if artifact and cfg.lora_r <= 0 and cfg.save_student:
        tok.save_pretrained(artifact)
    if artifact:
        print(f"[save] {'adapter' if cfg.lora_r > 0 else 'student'} -> {artifact}",
              flush=True)
