...
Формула:$$L(N, D) = E + \frac{A}{N^\alpha} + \frac{B}{D^\beta}$$где $E$ — неустранимый шум (irreducible loss), $A, B, \alpha, \beta$ — эмпирические константы ($\alpha \approx 0.34, \beta \approx 0.28$).
...
Формула:$$L(N, D) = L_0 + \sum_{k=1}^K \frac{A_k}{N^{\alpha_k}} + \sum_{k=1}^K \frac{B_k}{D_k^{\beta_k}}, \quad p^* = \arg\min_p L(N, p \cdot D_{\text{total}})$$
...
Формула:$$L(N, D, V) = \frac{A}{N^\alpha} + \frac{B}{D^\beta} + \frac{C}{V^\gamma} + L_0, \quad V^* \propto N^\delta \quad (\delta \approx 0.4 \text{--} 0.5)$$
...
Формула:$$D_{\text{eff}}(E) = D_{\text{unique}} \cdot E^\gamma \implies L(N, D_{\text{unique}}, E) = \frac{A}{N^\alpha} + \frac{B}{(D_{\text{unique}} \cdot E^\gamma)^\beta} + L_0 \quad (\gamma \approx 0.5 \text{--} 0.7)$$
...
Формула:$$L(N, D, Q) = \frac{A}{N^\alpha} + \frac{B}{(Q \cdot D)^\beta} + L_0$$
...
Формула:$$\text{TCO}(N, D) = C_{\text{train}}(N, D) + C_{\text{infer}}(N, Q_{\text{total}}), \quad (N^*, D^*) = \arg\min_{N,D} \text{TCO}(N, D)$$
...
Формула:$$L(N, D) = L_0 + \sum_{k=1}^K \frac{B_k}{(\rho_k \cdot D_k)^\beta}$$
